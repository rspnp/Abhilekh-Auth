...
func (a *API) Provider(ctx context.Context, name string, scopes string) (provider.Provider, conf.OAuthProviderConfiguration, error) {
	...
	case "keycloak":
		// Fork: OIDC discovery + the openid scope, for Authentik compatibility.
		// Re-expressed for upstream's 3-value signature (provider, config, error)
		// instead of the 2-value form this fork was written against.
		pConfig = config.External.Keycloak
		p, err = provider.NewKeycloakProvider(ctx, config.External.Keycloak, scopes)
	case "linkedin":
