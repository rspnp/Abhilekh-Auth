...
func (a *API) validateChallenge(r *http.Request, db *storage.Connection, factor *models.Factor, challengeID uuid.UUID) (*models.Challenge, error) {
	...
	// A challenge can only be verified once. We deliberately do NOT pin the verify
	// request to the challenge's originating IP, which upstream does here:
	// legitimate clients routinely change source IP between challenge and verify
	// (Cloudflare edge, IPv4/IPv6 dual-stack, mobile/NAT roaming), and that check
	// rejected valid TOTP codes. This is the fork's fix from
	// "fix(mfa): stop pinning TOTP/Phone verify to the challenge IP" (#1),
	// re-applied on top of upstream's validateChallenge refactor.
	if challenge.VerifiedAt != nil {
		return nil, apierrors.NewUnprocessableEntityError(apierrors.ErrorCodeMFAChallengeAlreadyVerified, "MFA challenge has already been verified")
	}
...
func (a *API) verifyPhoneFactor(w http.ResponseWriter, r *http.Request, params *VerifyFactorParams) error {
	...
	// No IP pinning here either — see validateChallenge. validateChallenge has
	// already rejected an already-verified challenge, so this second check only
	// ever added the IP comparison this fork removes.
