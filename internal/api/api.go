...
func NewAPIWithVersion(globalConfig *conf.GlobalConfiguration, db *storage.Connection, version string, opt ...Option) *API {
	...
	r.Route("/", func(r *router) {
		...
		r.With(api.requireAuthentication).Route("/user", func(r *router) {
			r.Get("/", api.UserGet)
			// Fork routes, expressed in upstream's limiter idiom rather than the
			// tollbooth/sharedLimiter calls they replaced — upstream moved rate
			// limiting behind api.limiterOpts, and the old symbols are gone.
			...
			r.Route("/change-password", func(r *router) {
				r.With(api.limitHandler(api.limiterOpts.User)).Post("/", api.UserChangePassword)
			})
